...
- **Identity (no password)**: the name IS the account. `POST /api/fantasy/register` (name + required fun-fact) and `POST /api/fantasy/login` (name only) live in `app/routers/fantasy.py`; both return a **Bearer token** (like the admin area — `make_fantasy_token(user_id)` in `app/auth.py`, an `<id>.<HMAC>` bound to `SESSION_SECRET`). The frontend keeps it in `localStorage` (`pingpong.fantasy.token`) and sends it as `Authorization: Bearer <token>`; that's what "remembered across visits" now means. **Why tokens, not a cookie**: fantasy used to ride the Starlette session cookie, but the deployed frontend calls the backend cross-origin, so the cookie was a third-party cookie that **iOS Safari blocks by default** — the page couldn't load teams on iPhone. A token in the `Authorization` header has no such problem. `GET /api/fantasy/me` gates login-vs-team; `require_fantasy_user` (verifies the token via `verify_fantasy_token`) guards the team endpoints. There is **no** server logout — logout is the frontend dropping its token. Names are stored trimmed with a lowercased unique `name_key` so casing/spacing never splits an account.
