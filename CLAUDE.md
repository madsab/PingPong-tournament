...
- **Frontend**: `src/api/public.ts` (typed client, no credentials); `src/components/LeaderboardSection`, `ScheduleSection` (played/to-play split, expands a row into) `MatchDetail`. All reuse the F1 load-state pattern + `tokens.css`; mounted after `StandingsSection` on `/` in §4 order. Gold stays reserved for the rank-1 team only.

All three features have tests on both sides. Shadcn/Magic UI/Tailwind are still NOT installed — plain CSS Modules on purpose (YAGNI). Update this file as more features get added.
